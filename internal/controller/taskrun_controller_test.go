...
var _ = Describe("TaskRun Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind TaskRun")
			err := k8sClient.Get(ctx, typeNamespacedName, taskrun)
			if err != nil && errors.IsNotFound(err) {
				resource := &taskrunv1alpha1.TaskRun{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance TaskRun")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &TaskRunReconciler{
				Client: k8sClient,
